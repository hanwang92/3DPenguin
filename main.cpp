...
const float LIGHT_MIN            = -100.0;
const float LIGHT_MAX            = 100.0;
...
void initGlui()
{
	...
	glui_panel = glui_joints->add_panel("Light source controls");
	
	glui_spinner = glui_joints->add_spinner_to_panel(glui_panel, "light x:", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::LIGHT_X));
	glui_spinner->set_float_limits(LIGHT_MIN, LIGHT_MAX, GLUI_LIMIT_CLAMP);
	glui_spinner->set_speed(SPINNER_SPEED);
	glui_spinner->set_float_val(10.0);
	
	glui_spinner = glui_joints->add_spinner_to_panel(glui_panel, "light y:", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::LIGHT_Y));
	glui_spinner->set_float_limits(LIGHT_MIN, LIGHT_MAX, GLUI_LIMIT_CLAMP);
	glui_spinner->set_speed(SPINNER_SPEED);
	glui_spinner->set_float_val(10.0);
...
void display(void)
{
	...
	glTranslatef(joint_ui_data->getDOF(Keyframe::ROOT_TRANSLATE_X), 0, 0);
	glTranslatef(0, joint_ui_data->getDOF(Keyframe::ROOT_TRANSLATE_Y), 0);
	glTranslatef(0, 0, joint_ui_data->getDOF(Keyframe::ROOT_TRANSLATE_Z));
	...
	if (renderStyle==SOLID) {
		...
		//draw solid with polygon offset
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
...
void drawLegs() {
	glPushMatrix();
		...
		glPushMatrix();
			...
			glRotatef(joint_ui_data->getDOF(Keyframe::L_HIP_PITCH),	1,0,0);
			glRotatef(joint_ui_data->getDOF(Keyframe::L_HIP_YAW),	0,1,0);
			glRotatef(joint_ui_data->getDOF(Keyframe::L_HIP_ROLL),	0,0,1);
			glTranslatef(0.0,0.8,0.0);
			...
			glPushMatrix();
				...
				glRotatef(joint_ui_data->getDOF(Keyframe::L_KNEE),-1,0,0);
				glTranslatef(0.0,2.1,0.0);
				
				glTranslatef(-0.5,-2.1,0.7);
		...
		glPushMatrix();
			glTranslatef(0.0,-0.8,0.0);
			glRotatef(joint_ui_data->getDOF(Keyframe::R_HIP_PITCH),	1,0,0);
			glRotatef(joint_ui_data->getDOF(Keyframe::R_HIP_YAW),	0,1,0);
			glRotatef(joint_ui_data->getDOF(Keyframe::R_HIP_ROLL),	0,0,1);
			glTranslatef(0.0,0.8,0.0);
			...
			glPushMatrix();
				//Rotate About Joint 
				glTranslatef(0.0,-2.1,0.0);
				glRotatef(joint_ui_data->getDOF(Keyframe::R_KNEE),-1,0,0);
				glTranslatef(0.0,2.1,0.0);
...
void drawArms() {
	glPushMatrix();
		...
		glPushMatrix();
			...
			glRotatef(joint_ui_data->getDOF(Keyframe::L_SHOULDER_ROLL),	0,0,1);
			glTranslatef(1.4,-0.2,0.0);
		...
		glPushMatrix();
			...
			glTranslatef(1.4,0.2,0.0);
			glRotatef(joint_ui_data->getDOF(Keyframe::R_SHOULDER_PITCH),	1,0,0);
			glRotatef(joint_ui_data->getDOF(Keyframe::R_SHOULDER_YAW),	0,1,0);
			glRotatef(joint_ui_data->getDOF(Keyframe::R_SHOULDER_ROLL),	0,0,1);
			glTranslatef(-1.4,-0.2,0.0);
...
void drawHead() {
	glPushMatrix();
		...
		//Draw Beak
		glPushMatrix();
			glTranslatef(0,1.8,0.97);
			glRotatef(90.0,1,0,0);
			glScalef(0.5, 0.35, 0.15);
			if(wired)
				glColor3f(0.0, 0.0, 0.0);
			else
				glColor3f(1.0, 1.0, 0.0);
